...
class World():
    def __init__(self, data, world_i):
        ...
        self.exits_offsets, self.exits_values, self.exits_frames = getter_exits(data, world_i, self.nFrames)
        self.nExits = len(self.exits_frames)


    def randomize_exits(self, fix_boss_exit): #fix_boss_exit is a bool
        ...
        if fix_boss_exit:
            for i,this_exit_values in enumerate(self.exits_values):
                if this_exit_values[0] == self.bossFrame:
                    preboss_index = i
                    break
            new_order.remove(preboss_index)
            new_order.insert(preboss_index,preboss_index)


        new_exits_values = self.exits_values.copy()
        for i,this_exit_values in enumerate(new_exits_values):
            this_exit_values[0] = self.exits_values[new_order[i]][0]
            this_exit_values[4] = self.exits_values[new_order[i]][4]
            this_exit_values[5] = self.exits_values[new_order[i]][5]

        self.exits_values = new_exits_values
        return new_exits_values
